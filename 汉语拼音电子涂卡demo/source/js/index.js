	...
	$(function() { 
		...
		update:/*順序發生改變時，全局變量的順序也發生改變*/
			function(event,ui){
				...
				$("#sortable li").css("color","black");
				$("#sortable li:first").css("color","red");
				document.getElementById("contain").src="letter\\"+globalData[0]+".html";
			},
			tolerance:"pointer",
		}); 
	}); 
	...
	if(dir){/*列印切換*/
		if(id!=perId){
			if(id=="hdPic0"){
				...
				if(perOSD){
					document.getElementById("contain").src="letter\\"+perIframeIdx+".html";
				}
				changeOSD(perOSD);
			}else{
				document.getElementById("contain").src="letter\\"+dir;
...
function changeinnerFrame(frameIdx,objIdx){
			clearTimer();
			document.getElementById("contain").src="letter\\"+frameIdx+".html";
			if(perObjIdx!=objIdx){
...
function autoplay(){
	...
	if(globalData.length){
		for( i=0;i<globalData.length;i++){
			autoPlayTimer[i]=setTimeout("document.getElementById('contain').src='letter\\"+globalData[i]+".html';clearALLcolor();$('.red"+i+"').css('color','red');",3500*i);
			autoPlayFlage++;
		}
	}
	if(sessionStorage){
